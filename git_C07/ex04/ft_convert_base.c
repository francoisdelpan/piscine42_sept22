...
/*   Updated: 2022/10/06 14:29:09 by fpannier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_index(char *nbr, int *minus_count)
{
	int	i;
...
void	int_to_char(int nbr, char *base, char *result, int index)
{
	int	base_len;

	base_len = ft_strlen(base);
	if (nbr >= base_len)
		int_to_char(nbr / base_len, base, result, index - 1);
...
char	*ft_convert_base(char *nbr, char *base_from, char *base_to)
{
	...
	int_to_char(int_eq, base_to, result,
		result_len - 2 + minus_count);
	result[result_len - 1 + minus_count] = '\0';
	return (result);
}
